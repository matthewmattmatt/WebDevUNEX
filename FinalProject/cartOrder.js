...
function Customer( name, location, hasPaid ) {
	...
	this.showCustomerinHTML = function( id ) {
		$("#customerInfo").append( '<div id=#customerDiv>' + 'Customer Name: ' + this.name + '<br/>'
...
function Item( itemname, type, qty, unitPrice, i ) {
	...
	this.showLineItemsinHTML = function ( i ) {
		$("#orderInvoice").append( '<br/>' + '<div id="itemInvoice'+ i +'" class="invoice">'
		+ '<div class="name">' + this.itemname + '</div>' + " " + '<div class="type">' 
		+ this.type + '</div>' + " " + '<div id="quantity'+ i +'" class="qty">' + this.qty
		+ '</div>' + " "
		+ '<button id="down" type="button" onclick="adjustAmount('+ i +', -1);"> - </button>'
		+ " " + '<button id="up" type="button" onclick="adjustAmount('+ i +', 1);"> + </button>'
		+ " " + '<div class="price">' + this.unitPrice.toFixed(2) + '</div>'
		+ " " + '<div id="finalPrice' + i +'" class="totalCost">' + (unitPrice * qty).toFixed(2)
		+ '</div>' + "</div>" );
	}
}
